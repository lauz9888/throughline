...
- **Accessibility**: `@axe-core/playwright` is installed. For any new or changed page/flow, follow the pattern in `tests/e2e/home.spec.ts` and `tests/e2e/add-item-button.spec.ts`: `new AxeBuilder({ page }).withTags(WCAG_TAGS).analyze()`, then `expect(results.violations).toEqual([])`. `WCAG_TAGS` is the array defined in `.claude/STANDARDS.md`'s "WCAG conformance scope" section — read that file first and use its exact value; do not hardcode a second copy of the literal here. Since this runs in a real browser, it's the layer that catches color-contrast violations (deliberately skipped at the unit layer) — scan every meaningfully distinct visible state a real user would reach (e.g. a dropdown/modal in its open state), not just the page at load.
